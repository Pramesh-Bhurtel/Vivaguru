...
export const ReportCard: React.FC<ReportCardProps> = ({ report, session, onRestart }) => {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] py-10 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
      {/* Report Container */}
      <div
        ref={reportCardRef}
        className="bg-[#0f172a] border border-slate-800 rounded-sm p-6 sm:p-10 shadow-2xl space-y-8 relative overflow-hidden"
      >
        ...
        <div className="text-center space-y-2.5 pb-6 border-b border-slate-800 relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-sm bg-slate-900 border border-slate-800 text-indigo-400 text-[10px] font-mono uppercase tracking-[0.2em]">
            <Award className="w-3.5 h-3.5 text-indigo-400" />
            <span>Formal Examination Ledger &bull; DOSSIER-RECORD</span>
          </div>
          <h1 className="text-3xl sm:text-4xl text-white font-bold tracking-tight font-sans">
        ...
        {/* D3 Pie Chart Visual Confidence Score & Concept Mastery Display */}
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-0.5 rounded-sm uppercase tracking-wider">
        ...
        {/* Recharts Score Progression Across Individual Questions */}
        ...
        <div className="p-5 rounded-sm bg-slate-900/80 border-l-4 border-indigo-500 border-t border-r border-b border-slate-800 space-y-2">
        ...
        {calibration && calibration.totalRated > 0 && (
          <div className="bg-[#0a0f1d] border border-indigo-500/30 rounded-sm p-5 sm:p-6 space-y-4 shadow-lg">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-3">
              ...
              <span
                className={`text-[10px] font-mono uppercase tracking-wider px-2.5 py-1 rounded-sm border font-semibold ${
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-3 bg-slate-900 border border-slate-800 rounded-sm text-center">
            ...
            <div className="p-3.5 bg-slate-900/60 border border-slate-800 rounded-sm text-xs font-mono text-slate-300 flex items-start gap-2.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {/* Strengths */}
          <div className="bg-[#0a0f1d] border border-emerald-500/30 rounded-sm p-5 space-y-3">
          ...
          {/* Weak Spots */}
          <div className="bg-[#0a0f1d] border border-rose-500/30 rounded-sm p-5 space-y-3">
            ...
            <ul className="space-y-2.5 text-xs sm:text-sm text-slate-300 font-sans">
              {report.weakSpots.map((item, idx) => (
                <li key={idx} className="flex items-start gap-2.5">
                  <span className="text-rose-400 font-mono text-xs">0{idx + 1}</span>
                  <span>{item}</span>
                </li>
              ))}
            </ul>
        ...
        {/* Knowledge Gaps Analysis: Cross-Referenced Concepts Consistently Resulting in Lower Scores */}
        <KnowledgeGapsSection knowledgeGaps={knowledgeGaps} />

        {/* Chronological Expandable History: Previous Q&A Pairs, Scores, and Examiner Feedback */}
        ...
        {report.studySuggestions && report.studySuggestions.length > 0 && (
          <div className="bg-[#0a0f1d] border border-slate-800 rounded-sm p-5 space-y-3">
            <div className="flex items-center gap-2 text-xs font-mono font-bold uppercase tracking-[0.15em] text-indigo-400">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
              {report.studySuggestions.map((suggestion, idx) => (
                <div
                  key={idx}
                  className="p-3.5 rounded-sm bg-slate-900 border border-slate-800 text-xs text-slate-400 leading-relaxed font-sans"
                >
        ...
        {/* Action Buttons (Excluded from exported PDF) */}
        <div className="pdf-exclude-export pt-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-800">
          <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 text-xs font-mono uppercase tracking-wider px-5 py-2.5 rounded-sm border transition-all cursor-pointer font-semibold shadow-md ${
                isExportSuccess
            ...
            <button
              ...
              onClick={handlePrint}
              disabled={isExportingPdf}
              className="flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-slate-400 hover:text-white px-4 py-2.5 rounded-sm border border-slate-800 bg-slate-900 hover:bg-slate-800 transition-colors cursor-pointer"
            >
          ...
          <button
            ...
            onClick={onRestart}
            disabled={isExportingPdf}
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-sm bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-200 hover:text-white font-mono font-semibold text-xs uppercase tracking-[0.15em] transition-colors cursor-pointer"
          >
            <RotateCcw className="w-4 h-4" />
            <span>INITIALIZE NEW VIVA SESSION</span>
          </button>
        </div>
      </div>
    </div>
