...
import { Sparkles, AlertCircle, Shield, Flame, Smile, ArrowRight, BookOpen, Smartphone } from 'lucide-react';
...
export const InputScreen: React.FC<InputScreenProps> = ({
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex flex-col justify-center py-6 sm:py-10 px-3.5 sm:px-6 lg:px-8 max-w-5xl mx-auto pb-20 sm:pb-12">
      {/* Brand Hero with Official Logo & Tagline */}
      <div className="flex flex-col items-center text-center mb-6 sm:mb-8">
        <VivaGuruLogo variant="full" size="lg" className="mb-4" />

        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-900/80 border border-slate-800 text-indigo-400 text-[10px] font-mono uppercase tracking-[0.2em] mb-2 shadow-xs">
          <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse" />
          <span>Adaptive Socratic Examination Protocol</span>
        </div>

        <p className="text-slate-300 text-xs sm:text-sm md:text-base max-w-2xl mx-auto font-sans leading-relaxed">
      ...
      {savedSession && (
        <div className="mb-5 bg-gradient-to-r from-slate-900/90 via-indigo-950/40 to-slate-900/90 border border-indigo-500/40 p-4 sm:p-5 rounded-2xl shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 shrink-0 self-end sm:self-center">
            {onDiscardSavedSession && (
              <button
                ...
                className="text-xs font-mono text-slate-400 hover:text-rose-400 px-3.5 py-2 rounded-xl border border-slate-700 bg-slate-800/80 transition-colors uppercase tracking-wider active:scale-95 min-h-[44px]"
              >
                Discard
              </button>
            )}
      ...
      {/* Preset Topics Bar - Android Material Touch Cards */}
      <div className="mb-5 bg-[#0f172a]/80 backdrop-blur-xs border border-slate-800/80 p-4 sm:p-5 rounded-2xl shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
          {SAMPLE_TOPICS.map((topic, idx) => (
            <button
              ...
              className={`text-left p-3 sm:p-3.5 rounded-xl border transition-all text-xs flex flex-col justify-between active:scale-[0.98] min-h-[52px] ${
                activeTopicTitle === topic.title
                  ? 'border-indigo-500 bg-indigo-950/40 shadow-sm ring-1 ring-indigo-500/50 text-white'
            ...
            >
              <div className="flex items-center justify-between gap-2 mb-1.5">
                ...
                <span className="text-[9px] font-mono px-2 py-0.5 rounded-full bg-indigo-500/15 text-indigo-300 border border-indigo-500/25 uppercase">
                  {topic.category}
                </span>
              </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="bg-[#0f172a]/90 backdrop-blur-sm border border-slate-800/80 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-xl space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              ...
              <textarea
                ...
                className="w-full bg-[#0a0f1d] border border-slate-800 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl p-3.5 sm:p-4 text-slate-200 placeholder-slate-500 text-sm sm:text-base leading-relaxed font-sans resize-y transition-all min-h-[140px]"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
              ...
              <button
                ...
                className={`p-3.5 rounded-xl border text-left transition-all active:scale-[0.98] min-h-[64px] ${
                  difficulty === 'friendly'
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[9px] font-mono px-2 py-0.5 bg-emerald-400/10 text-emerald-400 border border-emerald-400/20 rounded-full font-bold">
                    GENTLE
                  </span>
                </div>
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[9px] font-mono px-2 py-0.5 bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 rounded-full font-bold">
                    FORMAL
                  </span>
                </div>
                <p className="text-[11px] text-slate-400 leading-relaxed">
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[9px] font-mono px-2 py-0.5 bg-rose-500/10 text-rose-400 border border-rose-500/20 rounded-full font-bold">
                    STRICT
                  </span>
                </div>
                <p className="text-[11px] text-slate-400 leading-relaxed">
          ...
          {/* Android-First High-Contrast Action Button */}
          <div className="pt-2">
            <button
              type="submit"
